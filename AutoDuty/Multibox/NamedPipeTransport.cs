...
namespace AutoDuty.Multibox
{
    ...
    public sealed class NamedPipeTransport(string pipeName, string serverName = ".") : ITransport
    {
        ...
        public void StopServer()
        {
            ...
            this.cts?.Cancel();
            // 取消後這些 task 會以 OperationCanceledException 收尾,WaitAll 會擲出
            // AggregateException。這裡只是要等它們真的結束好安全 Dispose,擲出的是
            // 預期中的取消例外,吞掉即可(不吞會讓 StopServer 的呼叫端炸掉)。
            try
            {
            ...
            {
                DebugLog("Ignoring exception while draining connect tasks: " + ex.Message);
            }
